...
export const getDashboardStats = async (req, res) => {
  try {
    ...
    const [totalIssued] = await db.execute('SELECT COUNT(*) as count FROM issued_books WHERE return_date IS NULL');
    const [totalOverdue] = await db.execute('SELECT COUNT(*) as count FROM issued_books WHERE due_date < CURDATE() AND return_date IS NULL');
    const [totalFines] = await db.execute('SELECT SUM(amount) as total FROM fines WHERE paid = FALSE');
